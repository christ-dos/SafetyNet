...
/**
 * Class integration tests of FireStation entity which verify that all classes
 * works correctly together to test the entity {@link FireStation}
 * 
...
public class FireStationTestIT {
	...
	public void testRequestGetFireStationExist_whenInputAddress951LoneTreeRd_thenReturnStatusIsOK() throws Exception {
		...
		// WHEN
		// THEN
		mockMvcFireStation.perform(get("/firestation/address?address=951 LoneTree Rd")).andExpect(status().isOk())
				.andExpect(jsonPath("$.address", is("951 LoneTree Rd"))).andExpect(jsonPath("$.station", is("2")))
				.andDo(print());
	}

	/**
	 * Method that test requestGet when fireStation not exist then throw a
	 * {@link FireStationNotFoundException} and the status of the request isNotFound
	 * 
	 * @throws Exception
	 */
	@Test
	public void testRequestGetFireStationNotExist_whenInputaddress95FlowerRd_thenThrowFireStationNotFoundException()
			throws Exception {
		// GIVEN
		// WHEN
		// THEN
		mockMvcFireStation.perform(get("/firestation/address?address=95 Flower Rd")).andExpect(status().isNotFound())
	...
	/**
	 * Method that test requestGet when input address is empty then throw a
	 * {@link EmptyFieldsException} and the status isBadRequest
	 *
	 * @throws Exception
	 */
	@Test
	...
	/**
	 * Method that test requestPost when fireStation to save already exist then
	 * throw {@link FireStationAlreadyExistException}
	 *
	 * @throws Exception
	 */
	@Test
	public void testRequestPost_whenFireStationAlreadyExist_thenThrowFireStationAlreadyExistException()
			throws Exception {
		// GIVEN
		...
		mockMvcFireStation
				...
				.andExpect(jsonPath("$.message",
						is("The FireStation that we try to save already exist, please proceed to an update")))
				.andExpect(
	...
	public void testRequestPost_whenFireStationNotExist_shouldSaveFireStation() throws Exception {
		...
		mockMvcFireStation
				.perform(MockMvcRequestBuilders.post("/firestation")
						...
						.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andExpect(jsonPath("$.address", is("25 Blood St")))
				.andExpect(jsonPath("$.station", is("5"))).andDo(print());
	}

	/**
	 * Method that test requestDelete when fireStation to delete exist then return a
	 ...
	 */
	@Test
	public void testRequestDelete_whenFireStationExistWithAddress947ERoseDr_thenReturnMessageSUCCESS()
			throws Exception {
		// GIVEN
		// WHEN
		// THEN
		mockMvcFireStation.perform(delete("/firestation?address=947 E. Rose Dr")).andExpect(status().isOk())
				.andExpect(jsonPath("$", is("SUCCESS"))).andDo(print());

		mockMvcFireStation.perform(get("/firestationAddress?address=947 E. Rose Dr")).andExpect(status().isNotFound())
				.andExpect(jsonPath("$.address").doesNotExist()).andExpect(jsonPath("$.station").doesNotExist())
				.andDo(print());
	}

	/**
	 * Method that test requestDelete when fireStation to delete not exist then
	 * return a String "FireStation cannot be deleted"
	 * 
	 *
	 * @throws Exception
	 */
	@Test
	public void testRequestDelete_whenFireStationNotExist__thenReturnMessageFireStationCannotBeDeleted()
			throws Exception {
		// GIVEN
		// WHEN
		// THEN
		mockMvcFireStation.perform(delete("/firestation?address=5 Portugal Rd")).andExpect(status().isOk())
				.andExpect(jsonPath("$", is("FireStation cannot be deleted"))).andDo(print());
	}

	/**
	 * Method that test requestPut when fireStation to update exist then return
	 * fireStation with the field station updated with "5"
	 * 
	 *
	 * @throws Exception
	 */
	@Test
	public void testRequetePut_whenTheFireStationExistAndfieldStationIsUpdated_thenVerifyThatFieldStationUpadtedWithFive()
			throws Exception {
		// GIVEN
		FireStationControllerTest fireStationControllerTest = new FireStationControllerTest();
		...
		mockMvcFireStation
				...
				.andExpect(jsonPath("$.station", is("15"))).andDo(print());
	}

	/**
	 * Method that test requestPut when fireStation to update not exist then throw a
	 * {@link FireStationNotFoundException}
	 *
	 * @throws Exception
	 */
	@Test
	public void testRequetePut_whenFireStationToUpdateNotExist_thenThrowFireStationNotFoundException()
			throws Exception {
		// GIVEN
		FireStationControllerTest fireStationControllerTest = new FireStationControllerTest();
	 ...
	 */
	@Test
	public void testRequetePut_whenInputFieldIsInvalid_shouldReturnMethodArgumentNotValidExceptionMustNotBeBlank()
			throws Exception {
		// GIVEN
		FireStationControllerTest fireStationControllerTest = new FireStationControllerTest();
